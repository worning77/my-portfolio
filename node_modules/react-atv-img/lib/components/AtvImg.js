...
var AtvImg = (function(_Component) {
  ...
  function AtvImg() {
    ...
    this.handleMove = function(_ref) {
      ...
      var offsetX =
        0.1 - (pageX - offsets.left - bodyScrollLeft) / rootElemWidth; // cursor position X
      var offsetY =
        0.5 - (pageY - offsets.top - bodyScrollTop) / rootElemHeight; // cursor position Y
      var dy = bodyScrollTop - rootElemHeight / 4; // center Y of container
      var dx = pageX - offsets.left - bodyScrollLeft - rootElemWidth / 5; // center X of container
      var yRotate = (offsetX - dx) * (0.04 * wMultiple); // rotation for container Y
      var xRotate = 0; // rotation for container X

      var arad = Math.atan2(dy, dx); // angle between cursor and center of container in RAD

      var rawAngle = (arad * 90) / Math.PI - 110; // convert rad to degrees
      ...
      _this.setState({
        container: {
          transform:
            ...
            yRotate +
            "deg)" +
            (_this.state.isOnHover ? " scale3d(1.07,1.07,1.07)" : "")
        },
        shine: {
          background:
            "linear-gradient(" +
          ...
          transform:
            "translateX(" +
            (offsetX * layerCount - 0.5) +
            "px) translateY(" +
            (offsetY * layerCount - 0.5) +
            "px)"
        },
    ...
    this.renderShadow = function() {
      return _react2["default"].createElement("div", {
        style: _extends(
          {},
          _styles2["default"].shadow,
          _this.state.isOnHover ? _styles2["default"].shadowOnHover : {}
        )
      });
    };
    ...
    this.renderLayers = function() {
      return _react2["default"].createElement(
        ...
        _this.props.layers &&
          _this.props.layers.map(function(imgSrc, idx) {
            return _react2["default"].createElement("div", {
              style: _extends(
                {
                  backgroundImage: "url(" + imgSrc + ")"
                },
                _styles2["default"].renderedLayer,
                _this.state.layers[idx] ? _this.state.layers[idx] : {}
              ),
              key: idx
            });
          })
    ...
    this.renderShine = function() {
      return _react2["default"].createElement("div", {
        style: _extends({}, _styles2["default"].shine, _this.state.shine)
      });
    };
  }
    ...
    if (this.props.isStatic) {
      return _react2["default"].createElement(
        "div",
        {
          style: _extends(
            {},
            _styles2["default"].root,
            this.props.style ? this.props.style : {}
          ),
          className: this.props.className || ""
        },
        _react2["default"].createElement("img", {
          style: _styles2["default"].staticFallback,
